...
using BlazorPdfApp.Services;               // <-- NEW: สำหรับ PdfBrowserService
using System.Reflection;                   // <-- NEW: scanner
using System.Runtime.Loader;               // <-- NEW: scanner
...
builder.Services.AddSingleton<List<BlazorPdfApp.Hosting.PluginRegistration>>();
builder.Services.AddSingleton<IReadOnlyList<BlazorPdfApp.Hosting.PluginRegistration>>(sp => sp.GetRequiredService<List<BlazorPdfApp.Hosting.PluginRegistration>>());
...
using (var scope = app.Services.CreateScope())
{
    ...
    // ========= [NEW] SCAN: log assembly/entryType ที่ implement IBlazorPlugin =========
    try
    {
        var dlls = Directory.Exists(pluginsDir)
            ? Directory.EnumerateFiles(pluginsDir, "*.dll", SearchOption.AllDirectories).ToList()
            : new List<string>();

        if (dlls.Count == 0)
            pluginLogger.LogWarning("[PluginScan] No .dll files under {dir}", pluginsDir);
        else
            pluginLogger.LogInformation("[PluginScan] Scanning {count} dll(s) in {dir}", dlls.Count, pluginsDir);
        ...
        foreach (var dll in dlls)
        {
            try
            {
                ...
                {
                    if (typeof(IBlazorPlugin).IsAssignableFrom(t) && !t.IsAbstract && !t.IsInterface)
                    {
                        foundAny = true;
                        pluginLogger.LogInformation(
                            "[PluginScan] FOUND -> assembly={assembly}, entryType={entryType}",
                            asmName, t.FullName
                        );
                    }
                }
                ...
                if (!foundAny)
                    pluginLogger.LogDebug("[PluginScan] {assembly}: no IBlazorPlugin types", asmName);
            }
            catch (Exception ex)
            {
                pluginLogger.LogWarning(ex, "[PluginScan] Failed to load {dll}", dll);
            }
        }
    }
    catch (Exception ex)
    {
        pluginLogger.LogWarning(ex, "[PluginScan] Unexpected error");
    }
    // ========= [/NEW] SCAN =========

    // โหลดจริงด้วย PluginLoader
    var loaded = PluginLoader.LoadAll(services, pluginsDir);

    var descriptorStore = services.GetRequiredService<List<BlazorPdfApp.Hosting.PluginRegistration>>();
...
// ============================================================================
// ============  ใส่ Service/records ทั้งหมดไว้ใน namespace ชัดเจน  ==========
// ============================================================================
namespace BlazorPdfApp.Services
{
    ...
    internal sealed class PdfBrowserService
    {
        ...
        internal async Task<IResult> UploadPdfAsync(string line, HttpRequest request)
        {
            ...
            try
            {
                ...
                versions.Sort((l, r) => l.Division.CompareTo(r.Division));
        ...
        {
            if (metadata.Documents.Count == 0) return false;
            ...
            foreach (var key in metadata.Documents.Keys.ToList())
            {
                ...
                var filtered = versions.Where(v => fileSet.Contains(v.FileName)).ToList();

                if (filtered.Count != versions.Count)
                {
                    changed = true;
                    if (filtered.Count == 0) metadata.Documents.Remove(key);
                    else metadata.Documents[key] = filtered;
                }
            }
        ...
        {
            try { SaveMetadata(directoryPath, metadata); }
            catch (Exception ex) when (ex is IOException or UnauthorizedAccessException)
            {
        ...
        {
            try { return File.GetLastWriteTimeUtc(fullPath); }
            catch (Exception ex) when (ex is IOException or UnauthorizedAccessException)
            {
        ...
        {
            if (string.IsNullOrWhiteSpace(rawBaseName)) return "Document";
        ...
        private static string StripDivisionSuffix(string baseName)
        {
            ...
            if (index >= 0)
            {
                ...
                if (int.TryParse(suffix, out _)) return baseName[..index];
            }
            return baseName;
        }
        ...
        private static string SanitizeFileNameComponent(string value)
        {
            ...
            foreach (var ch in value) builder.Append(invalid.Contains(ch) ? '_' : ch);
        ...
        {
            linePath = null;
            if (!_allowedLines.Contains(line)) return false;
        ...
        {
            filePath = null;
            if (!IsValidPdfFileName(file)) return Results.BadRequest("Invalid PDF file name");
        ...
        private static bool IsValidFolderName(string? folderName)
        {
            ...
            if (string.Equals(trimmed, ".", StringComparison.Ordinal) || string.Equals(trimmed, "..", StringComparison.Ordinal)) return false;
        ...
        {
            if (segments.Count == 0) return fileName;
        ...
        private static string DescribeRelativeTime(DateTime timestampUtc)
        {
            ...
            if (delta.TotalMinutes < 1) return "เมื่อสักครู่";
            if (delta.TotalHours < 1) return $"ประมาณ {Math.Floor(delta.TotalMinutes)} นาทีที่แล้ว";
            if (delta.TotalDays < 1) return $"ประมาณ {Math.Floor(delta.TotalHours)} ชั่วโมงที่แล้ว";
            if (delta.TotalDays < 7) return $"ประมาณ {Math.Floor(delta.TotalDays)} วันที่แล้ว";
            if (delta.TotalDays < 30) return $"ประมาณ {Math.Floor(delta.TotalDays / 7)} สัปดาห์ที่แล้ว";
            if (delta.TotalDays < 365) return $"ประมาณ {Math.Floor(delta.TotalDays / 30)} เดือนที่แล้ว";
        ...
        private BranchEditStatus BuildBranchStatus(DirectoryInfo directory, IReadOnlyList<string> segments)
        {
            ...
            var documentStatuses = documentListings
                .Select(doc =>
                {
                    ...
                    DateTime? latest = versions.Count > 0 ? versions[0].UploadedUtc : (DateTime?)null;
            ...
            if (metadataChanged) TryPersistMetadata(directory.FullName, metadata);
            ...
            DateTime? lastModified = pdfFiles.Count > 0 ? pdfFiles.Max(file => file.LastWriteTimeUtc) : (DateTime?)null;
        ...
        // ใส่ภายใน class PdfBrowserService (ข้างใน namespace BlazorPdfApp.Services)
